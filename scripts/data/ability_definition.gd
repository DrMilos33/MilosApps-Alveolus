...
static func catalog() -> Dictionary:
	return {
		&"ability_focus_field": create(
			...
			"Behandlung priorisiert den Bereich und wirkt dort 25 % stärker."
		),
		&"ability_emergency_support": create(
			...
			"Stellt 14 Zustand wieder her und erzeugt 8 Schutz."
		),
		&"ability_defense_burst": create(
			...
			"Schädigt und verdrängt Bakterien im Zielbereich."
		),
		&"ability_treatment_line": create(
			...
			"Ein durchdringender Impuls in Cursorrichtung."
		),
		&"ability_protection_field": create(
			...
			"Bakterien im Feld bewegen sich langsamer und belasten den Zustand weniger."
		),
