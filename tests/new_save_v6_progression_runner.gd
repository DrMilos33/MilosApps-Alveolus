...
func _test_ranked_talent_contract() -> void:
	...
	_true(meta.purchase_talent_rank(&"manual_treatment_aim"), "Das Wurzeltalent kann gekauft werden")
	...
	_true(not meta.set_talent_active(&"manual_treatment_aim", false), "Eine belegte Voraussetzung wird nicht still entfernt")
	meta.clear_talents()
...
func _test_v6_roundtrip() -> void:
	...
	_true(source.set_talent_rank(&"manual_treatment_aim", 1), "Wurzeltalent wird gesetzt")
	...
	_true(source.set_talent_rank(&"piercing_return", 1), "Laserrückkehr wird gesetzt")
	...
	_equal(int(saved.get("talent_tree_revision", 0)), 3, "Save markiert Talentbaumrevision 3")
	...
	_true(restored.has_talent(&"piercing_return"), "Kompatible Aktivabfrage erkennt Rangtalente")
