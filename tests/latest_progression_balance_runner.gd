...
func _test_research_and_intro_rewards() -> void:
	...
	_true(not game._talents_unlocked(), "Introbelohnung allein schaltet den Talentbaum nicht frei")
	restored.intro_skipped = true
	_true(not game._talents_unlocked(), "Auch ein übersprungenes Intro schaltet den Talentbaum nicht frei")
	restored.register_level_result(_level_by_id(levels, &"early_localized_focus"), true, 300.0, 1, 20)
	_true(not game._talents_unlocked(), "Fall 1 lässt den Talentbaum weiterhin gesperrt")
	restored.register_level_result(_level_by_id(levels, &"localized_focus"), true, 300.0, 1, 30)
	_true(game._talents_unlocked(), "Erst der Abschluss von Fall 2 schaltet den Talentbaum frei")
	game.free()
...
func _test_boss_contract() -> void:
	...
	_equal(case_one.boss_projectile_pattern, &"normal", "Fall 1 verwendet genau ein normales Bossprojektil")
	_near(case_one.boss_projectile_speed_multiplier, 1.3, "Fall-1-Bossprojektile fliegen 30 Prozent schneller")
	_near(case_one.boss_projectile_turn_time_variation, 0.0, "Das normale Fall-1-Projektil besitzt keinen Richtungswechselvertrag")
	...
	_equal(case_two.boss_projectile_pattern, &"double_turn", "Fall 2 besitzt exklusiv die doppelte 90-Grad-Bahn")
	_near(case_two.boss_projectile_attack_speed_multiplier, 1.53, "Fall-2-Boss schießt relativ 15 Prozent langsamer")
	_near(case_two.boss_projectile_speed_multiplier, 1.5, "Fall-2-Bossprojektile fliegen 50 Prozent schneller")
	_near(case_two.boss_projectile_turn_time_variation, 0.10, "Fall-2-Richtungswechsel variieren deterministisch um bis zu zehn Prozent")
	...
	_near(case_three.boss_wave_length, 550.0, "Fall-3-Rautenprojektile bilden die zuletzt verlängerte Raute")
	...
	_near(case_four.boss_wave_amplitude, 115.0, "Fall-4-Rautenflugbahn ist um 25 Prozent breiter")
