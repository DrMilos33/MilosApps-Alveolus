...
func _test_shooting_lock_status_lifecycle() -> void:
	...
	_assert_equal(renderer.active_enemy_shooting_lock_count(), 1, "Die allgemeine dauerhafte Stoßsperre hält genau ein Verwirrtheitssymbol aktiv")
	permanent_enemy.step_fixed(30.0)
	_assert_equal(renderer.active_enemy_shooting_lock_count(), 1, "Das Symbol einer dauerhaften Schusssperre läuft nicht zeitlich aus")
	renderer.release_enemy(permanent_enemy, permanent_enemy.activation_generation)
	_assert_equal(renderer.active_enemy_shooting_lock_count(), 0, "Entity-Freigabe entfernt das dauerhafte Verwirrtheitssymbol synchron")
	permanent_enemy.recycle()
	_assert_true(not permanent_enemy.projectiles_suppressed(), "Pool-Recycling übernimmt keine dauerhafte Schusssperre")
