...
func _check_dynamic_boss_profiles(levels: Array[LevelDefinition]) -> void:
	...
	for level in levels:
		...
		_check(profile.get_projectile_pattern() == level.boss_projectile_pattern, "Bossprofil %s übernimmt das fallbezogene Projektilmuster" % level.id)
		...
		_check(is_equal_approx(profile.get_wave_length(), level.boss_wave_length), "Bossprofil %s übernimmt die aktuelle Rautenlänge" % level.id)
		...
		_check(target.boss_enemy_id == level.boss_enemy_id, "Bossprofil %s überträgt die aktuelle Gegner-ID in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_health_multiplier, level.boss_health_multiplier), "Bossprofil %s überträgt das aktuelle Leben in den Testlauf" % level.id)
		_check(is_equal_approx(target.enemy_speed_multiplier, level.enemy_speed_multiplier), "Bossprofil %s überträgt den aktuellen Fall-Tempoanteil in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_speed_multiplier, level.boss_speed_multiplier), "Bossprofil %s überträgt das aktuelle Bosstempo in den Testlauf" % level.id)
		_check(is_equal_approx(target.contact_damage_multiplier, level.contact_damage_multiplier), "Bossprofil %s überträgt den aktuellen Kontaktschaden in den Testlauf" % level.id)
		_check(target.boss_ranged_enabled == level.boss_ranged_enabled, "Bossprofil %s überträgt den aktuellen Fernkampfstatus in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_projectile_damage_multiplier, level.boss_projectile_damage_multiplier), "Bossprofil %s überträgt den aktuellen Projektilschaden in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_projectile_attack_speed_multiplier, level.boss_projectile_attack_speed_multiplier), "Bossprofil %s überträgt die aktuelle Schussrate in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_projectile_speed_multiplier, level.boss_projectile_speed_multiplier), "Bossprofil %s überträgt das aktuelle Projektiltempo in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_aura_screen_diameter_fraction, level.boss_aura_screen_diameter_fraction), "Bossprofil %s übernimmt den Auravertrag" % level.id)
		_check(target.boss_projectiles_require_empty_aura == level.boss_projectiles_require_empty_aura, "Bossprofil %s übernimmt die Projektilbedingung" % level.id)
		_check(target.boss_projectile_pattern == level.boss_projectile_pattern, "Bossprofil %s überträgt das Projektilmuster in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_projectile_turn_time_variation, level.boss_projectile_turn_time_variation), "Bossprofil %s überträgt die Richtungswechselvariation in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_wave_amplitude, level.boss_wave_amplitude), "Bossprofil %s überträgt die aktuelle Rautenbreite in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_wave_length, level.boss_wave_length), "Bossprofil %s überträgt die Rautenlänge in den Testlauf" % level.id)
		_check(target.boss_phase_minions == level.boss_phase_minions, "Bossprofil %s überträgt die aktuellen Phasenadds in den Testlauf" % level.id)
		_check(target.boss_phase_health_thresholds == level.boss_phase_health_thresholds, "Bossprofil %s überträgt die aktuellen Phasengrenzen in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_aura_speed_multiplier, level.boss_aura_speed_multiplier), "Bossprofil %s überträgt den aktuellen Aura-Tempobonus in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_aura_damage_multiplier, level.boss_aura_damage_multiplier), "Bossprofil %s überträgt den aktuellen Aura-Schadensbonus in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_reinforcement_interval, level.boss_reinforcement_interval), "Bossprofil %s überträgt das aktuelle Verstärkungsintervall in den Testlauf" % level.id)
		_check(target.boss_reinforcement_count == level.boss_reinforcement_count, "Bossprofil %s überträgt die aktuelle Verstärkungsgröße in den Testlauf" % level.id)
		_check(target.boss_reinforcement_minimum_phase == level.boss_reinforcement_minimum_phase, "Bossprofil %s überträgt die aktuelle Verstärkungsphase in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_add_defense_burst_shooting_lock_seconds, level.boss_add_defense_burst_shooting_lock_seconds), "Bossprofil %s überträgt die aktuelle Stoß-Schusssperre der Adds in den Testlauf" % level.id)
		_check(is_equal_approx(target.boss_add_projectile_attack_speed_multiplier, level.boss_add_projectile_attack_speed_multiplier), "Bossprofil %s übernimmt die Add-Schussrate" % level.id)
		_check(target.boss_count == profile.get_boss_count(), "Bossprofil %s überträgt die aktuelle Bossanzahl in den Testlauf" % level.id)
