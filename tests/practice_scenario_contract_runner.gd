...
func _check_dynamic_boss_profiles(levels: Array[LevelDefinition]) -> void:
	...
	for level in levels:
		...
		_check(target.boss_projectile_pattern == level.boss_projectile_pattern, "Bossprofil %s überträgt das Projektilmuster in den Testlauf")
		...
		_check(is_equal_approx(target.boss_wave_length, level.boss_wave_length), "Bossprofil %s überträgt die Rautenlänge in den Testlauf")
