...
func _test_case_one_normal_and_case_two_turn_variation() -> void:
	...
	if is_instance_valid(boss):
		...
		for shot_index in range(4):
			...
			var base_first: float = visible_width * 0.55 / 375.0
			var base_second: float = visible_width * 0.32 / 375.0
...
func _test_case_two_runtime_projectile_contract() -> void:
	...
	if is_instance_valid(boss):
		...
		if game.projectiles.size() > count_before:
			...
			var expected_first := visible_width * 0.55 / 375.0
			var expected_second := visible_width * 0.32 / 375.0
