...
func _test_catalog_and_config_contract() -> void:
	...
	var first_case_config := RunConfig.from_level(_level_by_id(&"early_localized_focus"))
	_near(first_case_config.regular_spawn_interval(0.0), 1.29027778, "Fall 1 verwendet das um zehn Prozent reduzierte Anfangstempo")
...
func _test_packet_balance_and_determinism() -> void:
	...
	_equal(first_count_a, 3, "Fall 1 beginnt nach der Reduktion mit einem kompakten Dreierpaket")
	...
	var first_late := StandardWaveDirector.new().configure(2201)
	_true(first_late.advance_clock(4.5), "Der späte Fall-1-Vergleich sammelt exakt einen Timeout")
	_equal(first_late.open_wave(first_config, 1.0, 145, true), 17, "Fall 1 endet nach der Reduktion mit dem seed-stabilen 17er-Paket")
