...
func _test_balance_and_movement_plumbing() -> void:
	...
	var enemies := ContentCatalog.enemy_definitions()
	_near(enemies[&"pneumococcus"].speed, 71.0, "Bakterium besitzt Geschwindigkeit 71")
	_near(enemies[&"bacterial_cluster"].speed, 71.0, "Bakteriengruppe besitzt Geschwindigkeit 71")
	_near(enemies[&"minor_focus"].speed, 32.0, "Kleiner Herd besitzt Geschwindigkeit 32")
	_near(enemies[&"infection_focus"].speed, 60.0, "Boss besitzt Geschwindigkeit 60")
