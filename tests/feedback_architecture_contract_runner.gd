...
func _test_balance_and_movement_plumbing() -> void:
	...
	var enemies := ContentCatalog.enemy_definitions()
	_near(enemies[&"pneumococcus"].speed, 85.0, "Bakterium besitzt Geschwindigkeit 85")
	_near(enemies[&"bacterial_cluster"].speed, 85.0, "Bakteriengruppe besitzt Geschwindigkeit 85")
	_near(enemies[&"minor_focus"].speed, 38.0, "Kleiner Herd besitzt Geschwindigkeit 38")
	_near(enemies[&"infection_focus"].speed, 72.0, "Boss besitzt Geschwindigkeit 72")
