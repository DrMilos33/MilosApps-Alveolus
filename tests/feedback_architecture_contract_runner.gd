...
func _test_reward_preview() -> void:
	...
	_equal(preview, 66, "Globale Forschungssteigerung skaliert die bestehende Niederlagenformel")
	_equal(MetaProgressionState.calculate_run_reward(false, 367.0, 6, 47, 1.35, 1), 82, "Ein besiegter Boss multipliziert die Endbelohnung")
	_equal(MetaProgressionState.calculate_run_reward(false, 367.0, 6, 47, 1.35, 2), 99, "Zwei Bosse liefern den additiven Bossmultiplikator")
