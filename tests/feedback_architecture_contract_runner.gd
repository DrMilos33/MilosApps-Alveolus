...
func _test_balance_and_movement_plumbing() -> void:
	...
	var enemies := ContentCatalog.enemy_definitions()
	_near(enemies[&"pneumococcus"].speed, 62.0, "Bakterium besitzt Geschwindigkeit 62")
	_near(enemies[&"bacterial_cluster"].speed, 62.0, "Bakteriengruppe besitzt Geschwindigkeit 62")
	_near(enemies[&"minor_focus"].speed, 28.0, "Kleiner Herd besitzt Geschwindigkeit 28")
	_near(enemies[&"infection_focus"].speed, 52.0, "Boss besitzt Geschwindigkeit 52")
