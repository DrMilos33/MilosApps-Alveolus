...
func _test_balance_and_movement_plumbing() -> void:
	...
	var enemies := ContentCatalog.enemy_definitions()
	_near(enemies[&"pneumococcus"].speed, 83.0, "Bakterium besitzt Tempo 83")
	_near(enemies[&"bacterial_cluster"].speed, 50.0, "Bakteriengruppe besitzt Tempo 50")
	_near(enemies[&"minor_focus"].speed, 12.0, "Kleiner Herd besitzt Tempo 12")
	...
	_near(stats.movement_speed, 272.5, "Bewegungsforschung addiert drei Prozent je Rang auf die Basis")
	...
	_near(stats.movement_speed, 272.5 * pow(1.05, 3), "Mobilitätsausbau multipliziert den aufgelösten Bewegungsstat je Rang")
	_equal(TherapyAvatar.MOVE_SPEED, 250.0, "Avatar-Fallback entspricht der neuen Doctor-Basis")
...
func _test_stat_sections_and_headings() -> void:
	...
	_true(serialized.contains("Stufe 16"), "Behandlungsreichweite wird als Stufe an die UI geliefert")
