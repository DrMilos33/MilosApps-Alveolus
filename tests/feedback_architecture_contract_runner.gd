...
func _test_balance_and_movement_plumbing() -> void:
	...
	var enemies := ContentCatalog.enemy_definitions()
	_near(enemies[&"pneumococcus"].speed, 54.0, "Bakterium besitzt Geschwindigkeit 54")
	_near(enemies[&"bacterial_cluster"].speed, 54.0, "Bakteriengruppe besitzt Geschwindigkeit 54")
	_near(enemies[&"minor_focus"].speed, 24.0, "Kleiner Herd besitzt Geschwindigkeit 24")
	_near(enemies[&"infection_focus"].speed, 45.0, "Boss besitzt Geschwindigkeit 45")
