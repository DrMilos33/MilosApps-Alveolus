...
func _test_balance_and_movement_plumbing() -> void:
	...
	_near(treatments[&"treatment_precision"].base_damage, 10.0, "Impuls besitzt den um drei reduzierten ganzzahligen Basisschaden")
	...
	var enemies := ContentCatalog.enemy_definitions()
	_near(enemies[&"pneumococcus"].speed, 94.0, "Bakterium besitzt Geschwindigkeit 94")
	_near(enemies[&"bacterial_cluster"].speed, 94.0, "Bakteriengruppe besitzt Geschwindigkeit 94")
	_near(enemies[&"minor_focus"].speed, 42.0, "Kleiner Herd besitzt Geschwindigkeit 42")
	_near(enemies[&"infection_focus"].speed, 79.0, "Boss besitzt Geschwindigkeit 79")
	_near(enemies[&"pneumococcus"].player_push_speed_multiplier, 2.0, "Kleine Bakterien verdoppeln nur ihr Doctor-seitiges Schiebetempo")
	_near(enemies[&"bacterial_cluster"].player_push_speed_multiplier, 1.5, "Bakteriengruppen erhöhen nur ihr Doctor-seitiges Schiebetempo um die Hälfte")
	var stats := PlayerStats.new()
	stats.configure_prepared_treatment(treatments[&"treatment_precision"])
	stats.apply_meta_progression({&"movement_training": 3})
	_near(stats.movement_speed, 186.0, "Galoppforschung löst auf einen ganzzahligen Wert auf")
...
func _test_reward_preview() -> void:
	...
	_equal(preview, 99, "Der zusätzliche Run-Faktor erhöht die bestehende Niederlagenformel exakt um 50 Prozent")
	_equal(MetaProgressionState.calculate_run_reward(false, 367.0, 6, 47, 1.35, 1), 123, "Ein besiegter Boss multipliziert die erhöhte Endbelohnung")
	_equal(MetaProgressionState.calculate_run_reward(false, 367.0, 6, 47, 1.35, 2), 148, "Zwei Bosse liefern weiterhin den additiven Bossmultiplikator")
