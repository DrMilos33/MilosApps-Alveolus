...
func _test_balance_and_movement_plumbing() -> void:
	...
	_near(stats.movement_speed, 150.0 * 1.09, "Geschwindigkeitsforschung addiert drei Prozent je Rang auf Basis 150")
	...
	_near(stats.movement_speed, 150.0 * 1.09 * pow(1.05, 3), "Mobilitätsausbau multipliziert den aufgelösten Geschwindigkeitsstat je Rang")
	_equal(PlayerStats.BASE_MOVEMENT_SPEED, 150.0, "Doctor-Basisgeschwindigkeit ist zentral 150")
