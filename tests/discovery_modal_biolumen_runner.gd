...
func _run() -> void:
	...
	var discovery_delta := tooltip.panel.get_global_rect().get_center() - target.get_global_rect().get_center()
	_check(absf(discovery_delta.x) > target.size.x and absf(discovery_delta.y) > target.size.y, "Neue Entdeckungen liegen diagonal an einer Zielecke")
	...
	tooltip.present(longer_definition, target)
	await _settle()
	_check(not tooltip.is_compact_sheet_active(), "Neue Entdeckungen rekonstruieren auch bei wenig Platz kein zentriertes Alt-Sheet")
	_check(not _approximately_centered(tooltip.panel, host.size), "Die Entdeckung bleibt an einer entfernten Ecke statt das Ziel mittig zu überdecken")
	_check(_inside_viewport(tooltip.panel, host.size), "Die randgebundene Entdeckung bleibt vollständig im Viewport")
	...
	_check(not tooltip.is_compact_sheet_active(), "Auch ohne Ziel verwendet die Entdeckung die gemeinsame Eckplatzierung")
