...
func _test_build_state() -> void:
	...
	_assert_near(build.value(RunBuildState.TREATMENT_DAMAGE), 27.0, "Gameplay damage resolves to an integer after additions and multipliers")
	...
	_assert_near(build.value(RunBuildState.TREATMENT_DAMAGE), 27.0, "Upper clamp above the resolved value is deterministic")
	...
	_assert_near(build.value(RunBuildState.TREATMENT_DAMAGE, 0.0, PackedStringArray(["spread"])), 27.0, "Missing tags exclude a modifier")
	build.remove_modifier(&"cap")
	_assert_near(build.value(RunBuildState.TREATMENT_DAMAGE, 0.0, treatment.tags), 42.0, "Matching tags apply before the integer gameplay boundary")
