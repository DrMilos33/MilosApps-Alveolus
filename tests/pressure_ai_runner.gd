...
## Equivalent collision-resolution orders can route different individual bodies
## while preserving the same sustained front pressure. Keep the acceptance floor
## below the lowest approved 220-body run instead of requiring one exact route.
const MINIMUM_LINEAR_RELOCATION_EVENTS := 260
const MINIMUM_LINEAR_DISTINCT_RELOCATIONS := 80
const MINIMUM_LINEAR_FRONT_SIDE_SAMPLE_RATIO := 0.55
const MINIMUM_LINEAR_VISIBLE_FRONT_SIDE_AVERAGE := 3.25
const MINIMUM_LINEAR_REACHABLE_FRONT_SIDE_AVERAGE := 3.25
