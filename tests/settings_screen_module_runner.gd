...
func _assert_closed_focus_and_accessibility(screen: SettingsScreen) -> void:
	...
	var move_up_primary := screen.control_for_setting(&"binding.move_up.0")
	...
	var move_right_primary := screen.control_for_setting(&"binding.move_right.0")
	_check(_focus_target(move_up_primary, move_up_primary.focus_neighbor_bottom) == move_right_primary, "D-Pad nach unten folgt im dreispaltigen Raster demselben Tastaturplatz der nächsten visuellen Zeile")
	_check(_focus_target(move_right_primary, move_right_primary.focus_neighbor_top) == move_up_primary, "D-Pad nach oben kehrt räumlich zum selben Tastaturplatz zurück")
