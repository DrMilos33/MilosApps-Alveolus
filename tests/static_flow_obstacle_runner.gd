...
func _test_knockback_clears_mover_route_but_not_static_body() -> void:
	...
	obstacle.configure_static_stun(true)
	obstacle.apply_knockback(Vector2.RIGHT, 90.0, 0.28, 1.0)
	world.step_fixed(FIXED_DELTA)
	_true(obstacle.global_position.is_equal_approx(obstacle_position), "Auch eine ausdrückliche Stun-Ausnahme verschiebt das stationäre Hindernis nicht")
	_true(obstacle.is_stunned(), "Eine ausdrückliche Eventregel darf ein stationäres Hindernis betäuben")
