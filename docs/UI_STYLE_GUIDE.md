...
- Die einmalige Forschungsführung wird aus der tatsächlichen Gebäude-Grafik positioniert. Ihr Text steht kollisionsfrei diagonal links oberhalb und ein `↘` zeigt auf das Ziel; feste Weltkoordinaten oder ein Pfeil ins Leere sind unzulässig.
...
- Forschung verwendet auf der Zielansicht 1280×720 vier Spalten und reduziert bei kleinerer Breite responsiv. Talentbaum-Revision 5 beginnt mit `treatment_damage_training` und verzweigt in `spread_shotgun`, `manual_treatment_aim` und `piercing_persistence`; `spread_penetration` und `piercing_return` bleiben entfernt und erhalten keinen neuen sichtbaren Knoten. Die drei Äste besitzen gezeichnete Verbindungen und echte Voraussetzungen. Bei kleiner logischer Breite werden Äste auf zwei beziehungsweise eine Spalte reduziert und vertikal gescrollt.
...
- Vor Abschluss oder Überspringen des Intros bedeckt eine voll verankerte `ModalSheet`-Schlossfläche die gesamte Talent-Inhaltsfläche unterhalb der weiterhin erreichbaren Tabs. Verdeckte Knoten und Resetaktionen sind unsichtbar und nicht bedienbar.
...
- `Neuigkeiten anzeigen` steuert ausschließlich pausierende Entdeckungsinfos und ist standardmäßig inaktiv. Der Schalter nennt diese Wirkung im Hover und kann die Anzeigen ausdrücklich wieder aktivieren; Intro- und Campusführung bleiben unabhängig.
