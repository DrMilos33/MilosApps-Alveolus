...
Ein Klick auf eine Komponente darf niemals raten, welcher Platz gefüllt oder ersetzt wird. Das Einsatzkit verwendet drei feste, 82 Pixel hohe medizinische Sockel mit gerahmtem Komponentenmedaillon, Rollenname, Inhalt, kurzer Wirkung, Kapazitätschip und eigenem Statuspunkt. Auswahl verändert ihre Geometrie nicht. Der aktive Sockel erhält dauerhaft einen goldenen Rand und ist auf breiten Bühnen über eine kurze goldene Leitung mit dem Instrumentenfeld verbunden. Im gestapelten Kompaktmodus entfällt die Leitung; stattdessen wiederholt der Editorkopf Rollenname und Sockelmedaillon.

Das Instrumentenfeld zeigt passende Komponenten als feste, 88 Pixel hohe Instrumentkacheln mit 46-Pixel-Medaillon, Name, Kapazitätschip und explizitem Zustand wie `EINGESETZT` oder `IN AKTIV 2`. Ab 760 logischen Pixeln stehen sie zweispaltig, darunter einspaltig. Der aktuelle Inhalt darf dort als klarer Status erscheinen, aber niemals wie eine neue, noch zu bestätigende Auswahl wirken. Verfügbare, anderweitig verwendete und gesperrte Einträge sind durch Symbol, Textzustand und Kontrast unterscheidbar; gesperrte Icons, Titel und Kosten sind vollständig entsättigt. Jede Planzeile besitzt mindestens 12 Pixel horizontalen Innenabstand. Die globale Kapazität wird im Plankopf als semantischer Wert farblich hervorgehoben. Kosten auf Plan- und Kandidatenzeilen bestehen nur aus der Zahl; das Kürzel `K` wird nicht verwendet. Der Inspektor nennt die unmittelbare Änderung vor dem direkten Ersetzen.
