...
- Das Lexikon verwendet zuerst eine dichte Vollflächen-Galerie statt einer schmalen Masterliste. Die fünf Reiter liegen ohne große Leerstufe direkt am oberen Inhaltsrand; Suche und Entdeckungsfortschritt teilen sich darunter eine kompakte Zeile. Jede kleine responsive Kachel zeigt Illustration beziehungsweise Icon und den echten Eintragsnamen. Gesperrte Kacheln ergänzen eine goldene Padlock-Glyphe und den konkreten Grund `Besiege diesen Gegner, um den Eintrag zu entdecken.` beziehungsweise `Entdecke diesen Eintrag im Spiel.`; sie verbergen medizinische, spielerische und statistische Details. Freigeschaltete Kacheln wiederholen keinen Zustandsfüller. Farbe allein trägt den Zustand nie. Bei 1280×720 sind mehrere Reihen und deutlich mehr als eine Handvoll Einträge gleichzeitig sichtbar. Nur Klick oder `Accept` öffnet das große Detail als eigene oberste Ebene und markiert den Eintrag dauerhaft. Maus-Hover, reiner Fokus und Suche verändern weder Auswahl noch Detail; `Cancel` schließt zuerst das Detail und kehrt in die Galerie zurück.
...
- Das Ergebnis besitzt keine Überschrift `Belohnung`. Sein Rewardstrip zeigt in vier gleichwertigen Spalten zuerst Forschungsicon und reine Zahl, danach vorläufig exakt `+ irgendwas`, `+ maybe nochwas` und `+ idk`. Ein Introergebnis zeigt dort nur beim ersten erfolgreichen Abschluss mit noch unverbrauchtem Grant 30 Forschung; Niederlagen und alle Ergebnisse nach dessen Verbrauch zeigen 0.
- Fähigkeitsschäden stehen im Ergebnis unter einem standardmäßig eingeklappten Disclosure. Die gesamte mindestens 44 Pixel hohe Kopfzeile ist fokussier- und anklickbar, heißt exakt `Fähigkeiten` und zeigt ihren Zustand zusätzlich mit zentraler Chevron-Glyphe sowie Accessible Name. Ausgeklappt folgen erst die Schadenszeilen und danach – sobald Fall 2 Talente freigeschaltet hat – aktive Talentränge beziehungsweise `Noch keine Talente aktiv`. Die einmalige Freischaltung `Talente freigeschaltet` bleibt zusätzlich im unteren Freischaltungsabschnitt sichtbar.
