...
- Mechanische Ausbauarten `Schaden`, `Breite` und `Radius` erhalten im Kartenheader je ein semantisches Icon-Text-Badge `SCHADEN`, `BREITE` beziehungsweise `RADIUS`. Das Badge wird aus Familien- und Vorschauwertdaten abgeleitet, nicht aus einzelnen Ausbau-IDs. Es ergänzt die Wirkung redundant, ersetzt weder die Raritätsdarstellung noch Komponententitel oder Fokus und erscheint bei anderen Familien nicht.
...
- `Hinweise anzeigen` ist der einzige sichtbare globale Schalter für optionale pausierende Entdeckungsinfos, die Campus-Forschungsführung und die Fall-1-Stoß-Zuweisungsführung. Für neue Spielstände und fehlende Settings ist er an; ausdrücklich gespeicherte Werte bleiben erhalten. Ausschalten oder `Hinweise ausblenden` in einer Führung wirkt sofort, schließt aktive optionale Hinweise, leert den Entdeckungsrückstau als gesehen und verhindert ein späteres Wiedererscheinen. Die bestätigungspflichtigen Introtexte bleiben davon unabhängig und bindend.
