...
- Intro- und Bossmeldungen verwenden dieselbe wiederverwendbare `PlainRunPrompt`-Darstellung als oberste UI-Ebene. Sie enthält ausschließlich Text, optional einen knappen Maushinweis, und niemals Panel, Shader, Hintergrund oder Button. Sie kann persistent ein- und ausgeblendet werden und hinterlässt beim Schließen oder Szenenwechsel weder Eingabefang noch sichtbaren Restzustand.
- Die Bossmeldung lautet `Infektionsherd erkannt`, steht korallrot direkt unter dem Lebensbalken und damit höher als bisher. Im Intro bleibt sie bis zur Linksklickbestätigung sichtbar; eine reguläre Bossmeldung darf ihren zeitgesteuerten Ablauf behalten, verwendet aber dieselbe containerlose View.
