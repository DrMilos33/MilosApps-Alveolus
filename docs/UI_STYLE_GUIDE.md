...
- Die Lexikon-Masterliste zeigt je Reiter eine Freischaltfortschrittszeile und ein Suchfeld. Jede Zeile kombiniert Illustration, Namen und den textlichen Zustand `Freigeschaltet`, bei Monstern `Besiegen zum Freischalten`, sonst `Entdecken zum Freischalten`; Farbe allein trägt den Zustand nie. Nur Klick oder `Accept` wählt einen Eintrag und markiert ihn dauerhaft. Maus-Hover, reiner Fokus und die Suche dürfen Auswahl, Pressed-State und Detailinhalt nicht verändern. Ohne Auswahl bleibt die vollständige Detailfläche als ruhige Anleitung sichtbar, statt zu einem dünnen Streifen zu kollabieren.
...
- `Neuigkeiten anzeigen` steuert ausschließlich pausierende Entdeckungsinfos und ist standardmäßig aktiv. Der Schalter nennt diese Wirkung im Hover; Intro- und Campusführung bleiben unabhängig.
