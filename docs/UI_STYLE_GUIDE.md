...
- Ab 920 logischen Pixeln zeigt die Einsatzplanung Loadout und Instrumentenfeld ungefähr im Verhältnis 44/56 nebeneinander. Darunter werden beide Bereiche in derselben Seiten-Scrollfläche gestapelt und nach einer Platzwahl automatisch zum Editor geführt; ein zusätzlicher Übersichts-Zwischenschritt ist unzulässig.
...
2. Ein Klick oder `Accept` auf einen der exakt drei sichtbaren Planplätze `Behandlung`, `Aktiv 1` und `Aktiv 2` wechselt ausdrücklich das Ziel. Maus-Hover darf dessen Kurzinfo als Tooltip zeigen; Tastatur und Gamepad rufen sie mit `ui_info` auf. Reiner Fokus zeigt keine Kurzinfo, und keine dieser Informationsaktionen ändert den Zielplatz.
...
Ein Klick auf eine Komponente darf niemals raten, welcher Platz gefüllt oder ersetzt wird. Das `LOADOUT` verwendet drei feste, 82 Pixel hohe medizinische Sockel mit gerahmtem Komponentenmedaillon, Rollenname, Inhalt, kurzer Wirkung, Kapazitätschip und eigenem Statuspunkt. Auswahl verändert ihre Geometrie nicht. Der aktive Sockel erhält dauerhaft einen goldenen Rand; eine zusätzliche Pfeil- oder Leitungsdekoration zwischen Loadout und Instrumentenfeld entfällt auf allen Größen. Der Editorkopf wiederholt Rollenname und Sockelmedaillon.

Das Instrumentenfeld zeigt passende Komponenten als feste, 88 Pixel hohe Instrumentkacheln mit 46-Pixel-Medaillon, Name, Kapazitätschip und explizitem Zustand wie `EINGESETZT` oder `IN AKTIV 2`. Ab 760 logischen Pixeln stehen sie zweispaltig, darunter einspaltig. Der aktuelle Inhalt darf dort als klarer Status erscheinen, aber niemals wie eine neue, noch zu bestätigende Auswahl wirken. Verfügbare, anderweitig verwendete und gesperrte Einträge sind durch Symbol, Textzustand und Kontrast unterscheidbar; gesperrte Icons, Titel und Kosten sind vollständig entsättigt. Jede Planzeile besitzt mindestens 12 Pixel horizontalen Innenabstand. Ungenutzte Kapazität und die Null eines leeren Sockels sind gedämpft; tatsächlich belegte Kapazität bleibt gold. Kosten auf Plan- und Kandidatenzeilen bestehen nur aus der Zahl; das Kürzel `K` wird nicht verwendet. `ENTFERNEN` bleibt eine sekundäre Aktion, erscheint aber als kompakte korallene Ausrüstungskapsel mit gerahmtem Icon statt als flacher Textlink. Der Inspektor nennt die unmittelbare Änderung vor dem direkten Ersetzen.
...
- Sobald Zufallsereignisse vorhanden sind, fasst der Fallheader die zwei Fallmerkmale in einer gemeinsamen Gruppe `ZUFALLSEREIGNISSE` zusammen und trennt sie durch eine ruhige goldene Linie von Dauer-/Bossfakten. In breiten Ansichten erhalten sie einen größeren gemeinsamen Horizontalraum und stehen nebeneinander; kompakt dürfen sie umbrechen. Fakten, vertikale Linie und Merkmalsgruppe liegen breit in einer Zeile, kompakt werden sie gestapelt und dieselbe Linie wird horizontal. Ohne Merkmale entfallen Gruppe und Separator vollständig.
...
- Upgradeicons werden über die betroffene Komponente datengetrieben aufgelöst. Breite Karten inszenieren sie als gerahmte 64-Pixel-Medaillons über dem kompakten Komponentennamen; bei 200-Prozent-UI wechselt dieselbe Karte in eine vollständige Querfassung mit 46-Pixel-Medaillon. Die UI besitzt keine ID-Mappingtabelle für zukünftige Ausbauten.
- Level-up-Karten lesen sich als Belohnungen statt als Produktkarten: Die Kartenwirkung bildet den großen zentralen Wertakzent auf der tatsächlichen Kartenachse, der Vorher-Nachher-Vergleich bleibt eine ruhige Begründungszeile und der äußere Modalträger tritt optisch hinter die drei Auswahlflächen zurück. Common, Magic und Rare behalten ihren vollständigen Seltenheitsrahmen und erhalten zusätzlich einen, zwei beziehungsweise drei kleine Raritätsdiamanten. Der ausgeschriebene Seltenheitsname bleibt im zugänglichen Kartennamen.
