...
- Das Ergebnis besitzt keine Überschrift `Belohnung`. Es beginnt mit einem klaren Ausgangsemblem und zeigt unmittelbar danach den Rewardstrip als Trophäenstufe. Dieser behält vier gleichwertige Plätze: zuerst das goldbetonte Forschungsicon mit reiner Zahl, danach vorläufig exakt `+ irgendwas`, `+ maybe nochwas` und `+ idk` als sichtbar gesperrte Zukunftsslots. Ein Introergebnis zeigt dort nur beim ersten erfolgreichen Abschluss mit noch unverbrauchtem Grant 30 Forschung; Niederlagen und alle Ergebnisse nach dessen Verbrauch zeigen 0. Danach folgen Begründung, drei kachellose Runkennzahlen sowie optionale Freischaltung und Meisterschaft. Auf der extrem kompakten 200-Prozent-Bühne darf ausschließlich das dekorative Ausgangsemblem entfallen; Titel und erste verdiente Belohnung bleiben beim Öffnen sichtbar. Im festen Footer steht `Fallübersicht` als einzige vollbreite Primäraktion über einer Zweierreihe aus `Erneut behandeln` und `Zum Campus`.
